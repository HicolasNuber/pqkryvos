...
To run the tests, run the container as
...
which generates a file `input.json` with content `{"a": 0, "b": 0}` in the respective folder. 
...
which generates files `witness.wtns` and `witness.json` in the respective folder. The file `witness.json` has content 
`["1", "0", "0", "0"]`.
...
###### 1. Proving Knowledge of an Opening of a BDLOP commitment
...
###### 2. Proving Ballot Validity of a BDLOP-committed ballot (over Goldilocks)
...
###### 3. Proving Correct Result Computation from aggregated BDLOP-committed ballots (over Goldilocks)
...
###### 4. Proving a Linear Relation between BDLOP-commited values (over Goldilocks)
