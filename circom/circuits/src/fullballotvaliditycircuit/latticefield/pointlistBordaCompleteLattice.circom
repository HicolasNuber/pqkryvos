...
include "../../voting/pointlistBorda.circom";

template computePointlistBordaCompleteLattice(bitsVotes,nVotes,nPoints,orderedPoints,N,q,n,k,comkey){
/**
 * Computes a commitment to a Pointlist Borda Vote, assuming SNARK field != Lattice Field.
 *
 *   Vote parameters:
 *   - bitsVotes: number of bits used to encode each vote.
 *   - nVotes: number of choices/candidates.
 *   - nPoints: number of assignable points/ranks
 *   - orderedPoints[nPoints]: Ordered list of assignable points; no duplicates allowed.
 *
 ...
 */
    ...
    component testPointlistBorda = assertPointlistBordaVoting(bitsVotes, nVotes, nPoints, orderedPoints);
    for (var j=0; j<numcommitments; j++){
        for (var i=0; i<N; i++){
            if(j*N+i < nVotes){
                testPointlistBorda.ballot[j*N+i] <== message[j][i];
            }
        }
    }
}