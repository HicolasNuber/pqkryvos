...
//Adapted to match query bound for goldilocks:
template<typename FieldT>
                                                ...
                                                const size_t max_tested_degree_bound)
{
    ...
    size_t query_bound = 0;
    if(this->make_zk_) {
        // Initial estimate for ZK case
        query_bound = 1;
    }

    // Compute query error numerator and query_error
    ...
    // Compute fractional proximity parameters
    this->calculate_encoded_ligero_proximity_parameters(query_bound);

    // Ensure total error is capped below 1
    long double total_error = query_error + this->fractional_encoded_ligero_proximity_parameter_;
    if(total_error >= 1.0L) total_error = 0.999L; // avoids log2(>0) > 0

    // Precompute number of query repetitions to achieve target bits
    const long double error_prob_target = powl(2.0L, -1.0L * query_soundness_bits);
    size_t num_query_repetitions = 1;
    long double accumulated_error = total_error;
    while(accumulated_error > error_prob_target) {
        num_query_repetitions += 1;
        accumulated_error = powl(total_error, num_query_repetitions);
    }

    this->encoded_ligero_params_.num_query_phase_repetitions_ = num_query_repetitions;

    // Set direct LDT parameters as before
    ...
    // For ZK mode, update query_bound if needed
    if(this->make_zk_) {
        this->query_bound_ = query_bound + 1;
    }
}
